...
export const MacbookScroll = ({
  ...
  const translate = useTransform(scrollYProgress, [0, 1], [0, 800]); // Decreased from 1200 to 800
  const rotate = useTransform(scrollYProgress, [0.1, 0.12, 0.3], [-28, -28, 0]); // Keep the same if rotation is unrelated to translation
  const textTransform = useTransform(scrollYProgress, [0, 0.3], [0, 50]); // Adjust this proportionally (previously was 100)
  const textOpacity = useTransform(scrollYProgress, [0, 0.2], [1, 0]); // No change needed if opacity is independent
